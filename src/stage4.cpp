...
void Stage4::Init(Scene_Game* scene_) {
	scene = scene_;
}
...
xx::Task<> Stage4::CoEnter() {
	
	scene->tasks.AddTask(scene->CoShowStageTitle("stage 4"));				// show stage title
...
xx::Task<> Stage4::CoCreateMonsters(int n1, int n2) {
	// i: num frames
	for (int i = 0; i < n1; i++) {
...
xx::Task<> Stage4::CoCreateMonsterTeam(int n, int64_t bonus) {
	// team death counter
